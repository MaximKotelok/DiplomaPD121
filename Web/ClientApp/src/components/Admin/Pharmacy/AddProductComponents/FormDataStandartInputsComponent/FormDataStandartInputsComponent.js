    ...
    return (
        <div>
            <InputForProductComponent
                ...
                onChange={a => { setFormData(a.target.name, a.target.value); }}
            />
            <InputForProductComponent
                className="margin-bottom"
                label="Короткий опис"
            ...
            <div className="margin-bottom select-div">
                <CustomSelectComponent
                    ...
                    options={listData.manufacturers.map(item => ({ value: item.id, label: item.name }))}
                    onChange={a => {setFormData("manufacturerID", a.value); }}
                />
                <CustomSelectComponent
                    ...
                    name="brandID"
                    placeholder="Бренд"
                    options={listData.brands.map(item => ({ value: item.id, label: item.name }))}
                    onChange={a => { setFormData("brandID", a.value) }}
                />
            </div>
            <div>
                {listData.mainAttribute && listData.mainAttribute.map && listData.mainAttribute.map((a,index) => {
                    if (a.list.length > 0)
                        return (
                            <div key={index} className='margin-bottom'>
                                <CustomSelectComponent
                                    ...
                                    onChange={b => {setFormData(a.name, b.value); }}                                    
