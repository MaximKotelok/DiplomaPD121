...
const MedicineTableComponent = ({ data }) => {
  ...
  if (loader !== StateInfos.LOADED) return <></>;
  return (
    <div>
      <p className={`${styles["section-title"]}`}>Особливості застосування</p>
      <div className={`${styles["table"]}`}>
        {data.map((a, index) => (
          <div
            key={index}
            className={`col-12 col-md-5 ${styles["table-item"]}`}
          >
            ...
            <div className={`${styles["block"]} col-6`}>
              <img
                ...
                src={`${ApiPath}${getPermission(a.id).pathToPhoto}`}
                alt="icon"
              />
              <span className={`${styles["permission-title"]}`}>
                {getPermission(a.id).title}
              </span>
            </div>
          </div>
        ))}
