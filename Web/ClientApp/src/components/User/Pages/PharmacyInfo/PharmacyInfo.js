...
const PharmacyInfo = (props) => {
  ...
  useEffect(() => {
    if (pharmacy) setIsFavoriteState(isFavoritePharmacy(pharmacy.id));
  }, [pharmacy]);

  useEffect(() => {
    if (loading === StateInfos.LOADED) {
      const myMap = L.map("pharmacyMap").setView(
      ...
      L.marker([pharmacy.latitude, pharmacy.longitude], { icon: icon })
        .addTo(myMap)
        .openPopup();
  ...
  return (
    <div>
      ...
      <p className={` ${styles["path-text"]} `}>
        Аптеки | Інформація про аптеку
      </p>
      <div className={`row`} style={{ paddingBottom: "40px" }}>
        <div className={`col-12 col-md-8 ${styles["pharmacy-info-container"]}`}>
          <h4 className={`${styles["title-pharnacy"]}`}>{pharmacy.title}</h4>
          <p className={`${styles["text-opus"]}`}>
            {isOpen
              ? `Відкрито до ${pharmacy.closeTime}`
              : `Буде відкрито з ${pharmacy.openTime}`}
          </p>
          <p className={`${styles["text-opus"]}`}>{pharmacy.address}</p>
          <p className={`${styles["text-opus"]}`}>
            Очікуваний час підтвердження броні{" "}
            <span className={`${styles["oclock-style"]}`}>
              {isOpen
                ? addMinutes(getCurrentTimeInUkraine(), 15)
                : addMinutes(pharmacy.openTime, 15)}
            </span>
          </p>

          <div className="row">
            <div className="p-0 col-12 col-md-6 pe-md-3">
              {" "}
              <Link
                to={`/map/pharmacies/${pharmacy.id}/${pharmacy.pharmaCompanyId}`}
                className={`btn w-100  brn-form ${styles["btn-style"]} ${styles["btn-prosta"]} `}
              >
                Аптеки {pharmacy.title} у місті {getCookie("city")}
              </Link>
            </div>
            <div className="p-0  col-12 col-md-6  ps-md-3">
              {" "}
        ...
        <div className={`col-12  col-md-4 mb-5 ${styles["pt-md-3"]}`}>
