        ...
        await cart[index].items.map(async (a) => {
          ...
          if (res.status === Success) {
            ...
            if (quantity > res.quantity) {
              ...
              toast.error(
                `Нажаль на складі є лише ${res.quantity} од. ${res.product.title}`
              );
            }
            return {
              id: res.id,
    ...
    if (isAuth) {
      res = await postLoggedReserve(
        productFormData.items.map((a) => {
          return { concreteProductId: a.id, quantity: a.quantity };
        }),
        cart[index].id
      );
    } else {
  ...
  return (
    ...
    >
      ...
      <div className={`col-md-12 col-xl-8 `}>
        ...
        >
          ...
          <p className={`mt-4 mb-2 ${styles["p-text-info"]}`}>
            {productFormData.isOpen
              ? `Відкрито до ${productFormData.closeTime}`
              : `Буде відкрито з ${productFormData.openTime}`}
          </p>
        ...
        <div
          className={`${styles["div-text-instruction"]} ${styles["div-block"]} mb-5`}
        >
